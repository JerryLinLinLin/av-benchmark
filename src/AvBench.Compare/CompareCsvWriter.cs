...
public static class CompareCsvWriter
{
    ...
    [
        ...
        "all_runs_mean_wall_ms",
        "baseline_all_runs_mean_wall_ms",
        "mean_wall_ms",
        "median_wall_ms",
        "first_run_wall_ms",
    ...
    public static async Task WriteAsync(IReadOnlyList<ComparisonRow> rows, string path, CancellationToken cancellationToken)
    {
        ...
        foreach (var row in rows)
        {
            builder.AppendLine(string.Join(",",
                ...
                row.AllRunsMeanWallMs.ToString("F1", CultureInfo.InvariantCulture),
                row.BaselineAllRunsMeanWallMs.ToString("F1", CultureInfo.InvariantCulture),
